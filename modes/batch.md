...
batch/
  ...
  batch-runner.sh               # Orchestrator script standalone
...
3. **Extract URLs**: Read DOM of results → extract list of URLs → append to `batch-input.tsv`
4. **For each pending URL**:
   a. Chrome: click on offer → read JD text from DOM
...
- If it dies → re-execute → reads `batch-state.tsv` → skip completed
- Lock file (`batch-runner.pid`) prevents double execution
- Each worker is independent: failure in offer #47 doesn't affect others
...
| Conductor dies | Re-execute → reads state → skip completed |
| PDF fails | Report .md saved. PDF remains pending |
