...
Process accumulated offer URLs in `data/pipeline.md`. The user adds URLs whenever they want, then runs `/career-ops pipeline` to process them all.
...
2. **For each pending URL**:
   ...
   d. **Execute complete auto-pipeline**: Evaluation A-F → Report .md → PDF (if score >= 3.0) → Tracker
