...
**If the input is JD text** (not URL): use directly, no need to fetch.

## Step 1 — Evaluation A-F
Execute exactly like `oferta` mode (read `modes/oferta.md` for all blocks A-F).
...
If final score is >= 4.0, generate draft answers for the application form:

1. **Extract form questions**: Use Playwright to navigate to the form and snapshot. If extraction fails, use generic questions.
...
**Tone rules:**
- **Confident without arrogance**: "I've spent the past year building LLM evaluation pipelines at research scale — your role is where I want to apply that experience next"
- **Selective without hubris**: "I've been intentional about finding a team where I can contribute meaningfully from day one"
...
- **Why this company?** → Mention something concrete about the company. "I've been following [product/research] since [context]."
